...
object TimeUsage {
  ...
  def timeUsageSummary(
    ...
    val workingStatusProjection: Column = when((df("telfs")>=1 && df("telfs")<3),"working").otherwise("not working")
      .as("working")
    ...
    val primaryNeedsProjection: Column = primaryNeedsColumns.reduce((x,y) => x+y).divide(60).as("primaryNeeds")
    val workProjection: Column = workColumns.reduce((x,y) => x+y).divide(60).as("work")
    val otherProjection: Column = otherColumns.reduce((x,y) => x+y).divide(60).as("other")
    df
  ...
  def timeUsageGroupedSqlQuery(viewName: String): String =
    "Select status, sex, age, ROUND(AVG(primaryNeeds),1) as primaryNeeds, ROUND(AVG(work),1) as work, ROUND(AVG " +
      s"(other),1) as other from  $viewName GROUP BY working, sex, age ORDER BY working, sex, age"
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    summed.groupByKey(x => (x.working, x.sex, x.age))
      .agg(typed.avg(x => x.primaryNeeds),
        typed.avg(x => x.work),
        typed.avg(x => x.other))
      .map{
